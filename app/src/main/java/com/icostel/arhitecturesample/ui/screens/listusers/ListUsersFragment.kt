...
class ListUsersFragment : BaseFragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        swipeRefreshLayout.setOnRefreshListener { listUsersViewModel.getUsersWithWorker(this) }
    ...
    override fun onCreateOptionsMenu(menu: Menu?, inflater: MenuInflater?) {
        ...
        searchCloseBtn?.setImageDrawable(resources.getDrawable(R.drawable.ic_close_primary_24dp, activity?.theme))
        searchCloseBtn?.setOnClickListener {
            if (!searchView.isIconified) {
                searchView.onActionViewCollapsed()
                shouldHandleBack = false
                setActionBarBackgroundColor(R.color.colorPrimary)
            }
        }
    ...
    private fun handleLoadingStatus(status: Status.Type?) {
        ...
        status?.let {
            when (status) {
                Status.Type.LOADING -> swipeRefreshLayout.isRefreshing = true
                Status.Type.SUCCESS -> {
